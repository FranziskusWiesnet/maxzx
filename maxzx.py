...
def M_0(f):
    k, h = quasi_polydivision(Poly(x, x), f)
    g = f + h * x
    L = poly(g, x).all_coeffs()
    if len(L) == 0:
        return True
...
def nu_1(f):
    ...
    c = f.subs(x, 0)
    try:
        return mod_inverse(c, 9701)
    except ValueError:
        return 0

def M_2(f):
    if f == 2*x-1 or f==x -1 or f==2*x-2 or f==-2*x+1 or f == (2*x-1)*(1-2*x):
        return True
    if f == 0:
        return True
    return False
...
if __name__ == '__main__':
    ...
    print(f"{a}; runtime: {(end - start)*1000:.3f} ms")

    start = time.perf_counter()
    a = unbounded_search(M)
    end = time.perf_counter()
    print(f"{a}; runtime: {(end - start) * 1000:.3f} ms")

    start = time.perf_counter()
    a=unbounded_search(M_0)
    end = time.perf_counter()
    print(f"{a}; runtime: {(end - start)*1000:.3f} ms")
    ...
    start = time.perf_counter()
    a = MaxZX(lambda _: False, lambda _: 1)
    end = time.perf_counter()
    print(f"{a}; runtime: {(end - start) * 1000:.3f} ms")
